...
export function BeachMap({ size, wind, swell, unit = "kt", showDirection = true }: Props) {
  const radius = size / 2;
    ...
    <View className="relative items-center justify-center" style={{ width: size, height: size }}>
      <Svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
        ...
        <G clipPath="url(#mapClip)">
          ...
          <Path d={`M0 ${size * 0.2} C${size * 0.25} ${size * 0.12} ${size * 0.55} ${size * 0.22} ${size} ${size * 0.15}`} stroke="#3D91BF" strokeWidth="18" opacity="0.55" />
          <Path d={`M0 ${size * 0.43} C${size * 0.3} ${size * 0.32} ${size * 0.58} ${size * 0.52} ${size} ${size * 0.39}`} stroke="#3D91BF" strokeWidth="16" opacity="0.45" />
          <Path d={`M0 ${size * 0.64} C${size * 0.25} ${size * 0.55} ${size * 0.62} ${size * 0.72} ${size} ${size * 0.59}`} stroke="#276E98" strokeWidth="18" opacity="0.45" />
        ...
        <SvgText x={radius} y={15} fill="#1E293B" fontSize="12" fontWeight="700" textAnchor="middle">N</SvgText>
        <SvgText x={size - 12} y={radius + 4} fill="#1E293B" fontSize="12" fontWeight="700" textAnchor="middle">E</SvgText>
        <SvgText x={radius} y={size - 7} fill="#1E293B" fontSize="12" fontWeight="700" textAnchor="middle">S</SvgText>
        <SvgText x={12} y={radius + 4} fill="#1E293B" fontSize="12" fontWeight="700" textAnchor="middle">W</SvgText>
      </Svg>
